/*
    ...
    press and hold the Home button of M5StickC on JoyC for 3 seconds to start
   scanning the hotspot of the car, and the pairing will be successful. After
   successful pairing, the link icon will be highlighted in the upper left
   corner of the screen, and the joystick value will be displayed on the screen.
   The left joystick controls the front and rear up and down, the left and right
   controls the pan, and the right joystick controls the left and right
   steering.
*/
...
void loop() {
  ...
  Disbuff.printf("%04d", adc_value[4]);
  // Disbuff.printf("%04X", adc_value[4]);
  Disbuff.setCursor(10, 115);
  Disbuff.printf("%04d", SendBuff[2]);
  Disbuff.pushSprite(0, 0);
}